	...
	(cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           (list-of-parameters (procedure-parameters procedure))
           (list-of-delayed-or-values (procedure-parameters procedure) arguments env) ; changed
           (procedure-environment procedure))))
	(else (error "Unknown procedure type: APPLY" 
		procedure))))

(define (list-of-parameters parameters)
	(map (lambda (x) 
		      (if (pair? x)
		      	  (car x)
		      	  x
...
(define (list-of-delayed-or-values parameters arguments env)
	(map (lambda (p a)
			(if (pair? p)
				(if (eq? (cadr p) 'lazy)
					(delay-it a env)
					(if (eq? (cadr p) 'lazy-memo)
						(delay-it-memo a env)
						(error "error input:" a)
					)
				)
...
(define (f a (b lazy) c (d lazy-memo)) body)
...
;;; L-Eval value:
(compound-procedure (a (b lazy) c (d lazy-memo)) (b) <procedure-env>)
